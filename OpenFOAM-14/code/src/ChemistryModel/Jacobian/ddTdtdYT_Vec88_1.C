/*---------------------------------------------------------------------------*\
  Description
      ...
      This variant (suffix 1) is specialised for nSpecie() % 8 == 1: the
      8-wide SIMD blocks cover the first Ns-1 species and the remaining single
      column and row ("tail", the last species) is handled by dedicated scalar
      tail blocks.
...
) const noexcept
{
    ...
    // ---- dT/dt = -sum_i Ha_i*dY_i/dt / CpM (vector part over the first Ns-1 species) ----
    ...
    {
        // Tail: add the last-species term i = Ns-1 to dTdt.
    ...
    // ---- d(dTdt)/dT: diagonal accumulator ddTdtdT = Jac[Ns][Ns] ----
    ...
    // ---- fill row nSpecie: Jac[Ns][i] = -( sum_j Ha_j*Jac[j][i] + Cp_i*dTdt ) / CpM ----
    for (int i=0; i<this->nSpecie()-1; i=i+8)
    {
        ...
        // Accumulate -sum_j Ha_j*Jac[j][cols] over the 8-wide species rows.
        ...
        {
            // Tail row j = Ns-1: add the last species row (vector columns part).
        ...
        // d(dTdt)/dT numerator terms for the current 8 columns:
    ...
    // ---- Tail: last species i = Ns-1 (scalar path, Ns % 8 == 1) ----
    {
        int i = this->nSpecie()-1;

        double ddTdtdYi0 = 0;

        // Accumulate -sum_j Ha_j*Jac[j][Ns-1] over the species rows j..j+7.
        for (int j=0; j<this->nSpecie()-1; j=j+8)
        {
        ...
        {
            // Tail row j = Ns-1: add the last row contribution.
            int j = this->nSpecie()-1;
            ddTdtdYi0 = ddTdtdYi0 - Ha[j+0]*Jac[(j+0)*(alignN)+(i+0)];
        }
        double Cpi0 = Cp[i+0];
