/*---------------------------------------------------------------------------*\
  Description
      ...
      This variant (suffix 4) is specialised for nSpecie() % 8 == 4: the
      8-wide SIMD blocks cover Ns-4 species and the remaining four columns
      and rows ("tail") are handled by a single 256-bit tail block.
...
) const noexcept
{
    ...
    // ---- tail: last four columns (species Ns-4 .. Ns-1) ----
    {
        int i = this->nSpecie()-4;
        __m256d ddTdtdYi03v = _mm256_setzero_pd();
