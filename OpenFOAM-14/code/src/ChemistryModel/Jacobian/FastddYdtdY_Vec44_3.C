...
This variant (suffix 3) is specialised for nSpecie() % 4 == 3: the last three rows/columns are handled by dedicated tail blocks.
...
) const noexcept
{
    ...
    for(int i=0; i<this->nSpecie()-3; i=i+4)
    {
        ...
        }
        {
            ...
            __m256d invWv = _mm256_blend_pd(load256d(&invW[j+0]),zerov,0b1000);
            __m256d dCjdYj = _mm256_mul_pd(rhoMv,invWv);
            __m256d rhoMvj_ = _mm256_blend_pd(load256d(&rhoMByRhoi[j+0]),zerov,0b1000);

            __m256d Jci0j03v = _mm256_blend_pd(load256d(&ddNdtByVdcT[(i+0)*(alignN)+j+0]),zerov,0b1000);
            __m256d ddNi0dtByVdYj = _mm256_mul_pd(Jci0j03v,dCjdYj);
            __m256d r0 = _mm256_mul_pd(rhoMvj_,dPhi0dtv);
            __m256d ddYi0dtdYj = _mm256_fmadd_pd(Wi0ByrhoMv,ddNi0dtByVdYj,r0);
            store256d(&Jac[(i+0)*(alignN)+j+0],ddYi0dtdYj);
