...
This variant (suffix 2) is specialised for nSpecie() % 4 == 2: the last two rows/columns are handled by dedicated tail blocks.
...
) const noexcept
{
    ...
    for(int i=0; i<this->nSpecie()-2; i=i+4)
    {
        ...
        }
        {
            ...
            __m128d invWv = load128d(&invW[j+0]);
            __m128d dCjdYj = _mm_mul_pd(_mm256_castpd256_pd128(rhoMv),invWv);
            __m128d rhoMvj_ = load128d(&rhoMByRhoi[j+0]);

            __m128d Jci0j03v = load128d(&ddNdtByVdcT[(i+0)*(alignN)+j+0]);
            __m128d ddNi0dtByVdYj = _mm_mul_pd(Jci0j03v,dCjdYj);
            __m128d r0 = _mm_mul_pd(rhoMvj_,_mm256_castpd256_pd128(dPhi0dtv));
            __m128d ddYi0dtdYj = _mm_fmadd_pd(_mm256_castpd256_pd128(Wi0ByrhoMv),ddNi0dtByVdYj,r0);
            store128d(&Jac[(i+0)*(alignN) + j+0],ddYi0dtdYj);

            __m128d Jci1j03v = load128d(&ddNdtByVdcT[(i+1)*(alignN)+j+0]);
            __m128d ddNi1dtByVdYj = _mm_mul_pd(Jci1j03v,dCjdYj);
            __m128d r1 = _mm_mul_pd(rhoMvj_,_mm256_castpd256_pd128(dPhi1dtv));
            ...
            __m128d r2 = _mm_mul_pd(rhoMvj_,_mm256_castpd256_pd128(dPhi2dtv));
            ...
            __m128d ddNi3dtByVdYj = _mm_mul_pd(Jci3j03v,dCjdYj);
            __m128d r3 = _mm_mul_pd(rhoMvj_,_mm256_castpd256_pd128(dPhi3dtv));
